...
namespace BrokenEvent.ILStrip
{
    ...
    {
        private Dictionary<Type, refl.ConstructorInfo> _types;
        ...
        {
            var types = this.GetType().Assembly.GetTypes().Where(t => typeof(Context).IsAssignableFrom(t) && t != typeof(Context)).ToArray();
            _types = new Dictionary<Type, refl.ConstructorInfo>();

            foreach (var t in types)
            {
                var tgtType = t.GetConstructors().First().GetParameters().First().ParameterType;

                _types.Add(tgtType, t.GetConstructor(new[] { tgtType }));

            }
        }

        public Context Create(object tgt)
        {
            if (_types.ContainsKey(tgt.GetType()))
            {
                return (Context)_types[tgt.GetType()].Invoke(new[] { tgt });
    ...
    public class FieldRefContext : GenericContext<FieldReference>
    {
        public FieldRefContext(FieldReference tgt) : base(tgt)
        {
            this.Name = tgt.FullName;
        }
    }
    public class MethodRefContext : GenericContext<MethodReference>
    {
        public MethodRefContext(MethodReference tgt) : base(tgt)
        {
            this.Name = tgt.FullName;
        }
    }
    public class VariableDefinitionContext : GenericContext<VariableDefinition>
    {
        public VariableDefinitionContext(VariableDefinition tgt) : base(tgt)
        {
            this.Name = tgt.Name;
        }
    }
    public class MethodContext : GenericContext<MethodDefinition>
    {
       
        public MethodContext(MethodDefinition tgt):base(tgt)
        {
    ...
    public class ParameterDefinitionContext : GenericContext<ParameterDefinition>
    {
        public ParameterDefinitionContext(ParameterDefinition tgt) : base(tgt)
        {
            this.Name = tgt.Name;
        }
    }
    public class FieldDefinitionContext : GenericContext<FieldDefinition>
    {

        public FieldDefinitionContext(FieldDefinition tgt) : base(tgt)
        {
    ...
    public class TypeDefinitionContext : GenericContext<TypeDefinition>
    {

        public TypeDefinitionContext(TypeDefinition tgt) : base(tgt)
        {
        ...
        {
            return "Class " + this.Name + " with " + this.Children.Count() + " methods" ;
        }
    }
    public class GenericContext<T> : Context
    {
        protected T GenericTarget { get; set; }

        public GenericContext(T tgt)
        {
            this.GenericTarget = tgt;
        }
        ...
        public override string ToString()
        {
            return base.ToString() + " with " + this.Children.Count() + " children";
        }
    }
    public class Context
    {
        ...
        protected virtual string GetTargetName()
        {
            ...
            var td = this.Target as TypeDefinition;
            if(td!=null)
            {
                return td.Module.FullyQualifiedName;
            }
            var mr = this.Target as MethodReference;
            if(mr!=null)
            {
                return mr.FullName;
            }
            var vd = this.Target as VariableDefinition;
            if(vd!=null)
            {
                return vd.Name;
            }
            var fd = this.Target as FieldDefinition;
            if(fd!=null)
            {
                return fd.Name;

            }
            var ca = this.Target as CustomAttribute;
            if(ca!=null)
            {
                return ca.AttributeType.Name;

            }
            var pd = this.Target as ParameterDefinition;
            if(pd!=null)
            {
                return pd.Name;
            }

            var tr = this.Target as TypeReference;
            if(tr!=null)
            {
                return tr.FullName;
            }

            var fr = this.Target as FieldReference;
            if(fr!=null)
            {
                return fr.FullName;
            }
        ...
        public IList<Context> Children { get; private set; }

        private List<object> _outRefs;

        public void AddOutRef (object tgt)
        {
            if (_outRefs == null) _outRefs = new List<object>();
            _outRefs.Add(tgt);
        ...
        public Context CreateChild(object tgt)
        {

            var ret = ContextFactory.Instance.Create(tgt);
            if (ret == null)
            {
                 ret = new Context();
                ret.Parent = this;
        ...
        private void AddChild(Context ctx)
        {
            if (Children == null) Children = new List<Context>();
    ...
    public class ILStrip
    {
        ...
        public void ScanUsedClasses()
        {
            ...
            for (int i = 0; i < arTypes.Count(); i++)
                WalkClass(arTypes[i], null );
        }
        private async Task WalkCustomAttributesAsync(Collection<CustomAttribute> customAttributes, Context context)
        {
            await Task.Run(() =>
            {
                int i = 0;
                while (i < customAttributes.Count)
                {
                    ...
                    {
                        AddUsedType(attribute.AttributeType, context.CreateChild(attribute));
                        i++;
                    }
                }
            });
        ...
        {
            WalkCustomAttributesAsync(customAttributes, null  ).Wait();
        }
        private async Task WalkClassAsync(TypeDefinition type, Context context)
        {
            ...
            while (true)
            {
                ...
                {
                    AddUsedType(baseDef, context.CreateChild(baseDef));
                    current = baseDef;
                }
                else
                {
            ...
            foreach (MethodDefinition method in type.Methods)
                await WalkMethodAsync(method ,context.CreateChild(method));

            await WalkCustomAttributesAsync(type.CustomAttributes, context.CreateChild(type));

            foreach (TypeReference iface in type.Interfaces)
                AddUsedType(iface.Resolve(), context.CreateChild(iface));

            foreach (FieldDefinition field in type.Fields)
            {
                var ctx = context.CreateChild(field);
                await WalkCustomAttributesAsync(field.CustomAttributes, ctx);
                AddUsedType(field.FieldType, ctx );
            }
        }
        private void WalkClass(TypeDefinition type, Context context)
        {
            WalkClassAsync(type, context).Wait();

        }
        private async Task WalkMethodAsync(MethodDefinition method, Context context)
        {
            await WalkCustomAttributesAsync(method.CustomAttributes, context );
            ...
            foreach (GenericParameter parameter in method.GenericParameters)
                AddUsedType(parameter.DeclaringType, context.CreateChild(parameter));

            foreach (ParameterDefinition parameter in method.Parameters)
            {
                var ctx = context.CreateChild(parameter);
                AddUsedType(parameter.ParameterType, ctx);
                await WalkCustomAttributesAsync(parameter.CustomAttributes, ctx);
            }

            AddUsedType(method.ReturnType, context );

            if (!method.HasBody)
                return;

            foreach (VariableDefinition variable in method.Body.Variables)
                AddUsedType(variable.VariableType, context.CreateChild(variable));
            ...
            foreach (Instruction instruction in method.Body.Instructions)
            {
                ...
                if (methodRef != null)
                {
                    var ctx = context.CreateChild(methodRef);
                    AddUsedType(methodRef.DeclaringType, ctx);
                    AddUsedType(methodRef.ReturnType, ctx);
                    ...
                    if (genericMethod != null)
                        foreach (TypeReference parameter in genericMethod.GenericArguments)
                            AddUsedType(parameter, ctx.CreateChild(parameter));

                    foreach (ParameterDefinition parameter in methodRef.Parameters)
                        AddUsedType(parameter.ParameterType, ctx.CreateChild(parameter));
                    continue;
                }
                ...
                if (typeRef != null)
                {
                    var ctx = context.CreateChild(typeRef);
                    foreach (GenericParameter genericParameter in typeRef.GenericParameters)
                        AddUsedType(genericParameter, ctx.CreateChild(genericParameter));
                    AddUsedType(typeRef.DeclaringType, ctx);
                    continue;
                }
                ...
                if (fieldRef != null)
                {
                    var ctx = context.CreateChild(fieldRef);
                    AddUsedType(fieldRef.FieldType, ctx);
                    AddUsedType(fieldRef.DeclaringType, ctx);
                }
            }
        }
        private void WalkMethod(MethodDefinition method)
        {
            WalkMethodAsync(method, null ).Wait();
        }

        private void AddUsedType(TypeReference typeRef, Context context )
        {
            if (typeRef == null)
                return;
            ...
            if (genericType != null)
                foreach (TypeReference reference in genericType.GenericArguments)
                    AddUsedType(reference, context.CreateChild(reference));
            ...
            foreach (GenericParameter parameter in typeDef.GenericParameters)
                AddUsedType(parameter, context.CreateChild(parameter));
        ...
        public async Task ListAllClassesAndVars()
        {
            ...
            var block = new ActionBlock<TypeDefinition>(
                async t => await WalkClassAsync(t, ctx.CreateChild(t)),
            ...
            using (var fout = File.CreateText(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "usedtypes.json")))
            {
                var settings = new JsonSerializerSettings() { ReferenceLoopHandling = ReferenceLoopHandling.Ignore,
                    Converters = new[] { new Context.ContextConverter() },
                    Formatting = Formatting.Indented };
                fout.WriteLine(JsonConvert.SerializeObject(ctx,  settings));
            }
        }
